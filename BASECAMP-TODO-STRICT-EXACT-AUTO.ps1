...
# Canonical single-resource URLs confirmed by Basecamp's own notification mails.
$TodoA = "https://app.basecamp.com/6259481/buckets/48506183/todos/10203340873"
$TodoB = "https://app.basecamp.com/6259488/buckets/48506260/todos/10203433510"
...
Write-Host "BASECAMP TODO STRICT EXACT RESOURCE AUTH MATRIX" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "A = Chrome / 6259481 / project 48506183"
Write-Host "B = Brave  / 6259488 / project 48506260"
Write-Host "Discovery yok. İki canonical /todos/<id> URL doğrudan test edilir." -ForegroundColor DarkGray
...
if (-not ($ProjectPassA -and $ProjectPassB)) {
    throw "PROJECT BASELINE başarısız. Exact To-do/cross çalıştırılmadı."
}

Start-Sleep -Milliseconds $DelayMs

Write-Host "`n[3/4] Exact single-To-do baseline..." -ForegroundColor Cyan
Write-Host "  A URL: $TodoA"
Write-Host "  B URL: $TodoB"

$TodoResultA = Invoke-ExactProbe -ClientId $ActorA.client_id -Url $TodoA -Marker $TodoMarkerA -Label "A -> A exact TODO"
$TodoResultB = Invoke-ExactProbe -ClientId $ActorB.client_id -Url $TodoB -Marker $TodoMarkerB -Label "B -> B exact TODO"

$TodoPassA = Test-Marker -Result $TodoResultA -Marker $TodoMarkerA
$TodoPassB = Test-Marker -Result $TodoResultB -Marker $TodoMarkerB

Write-Host "  A -> A TODO marker=$TodoPassA final=$($TodoResultA.final_url)"
Write-Host "  B -> B TODO marker=$TodoPassB final=$($TodoResultB.final_url)"

if (-not ($TodoPassA -and $TodoPassB)) {
    throw "EXACT TODO BASELINE başarısız. Cross-account jobs oluşturulmadı."
}

Start-Sleep -Milliseconds $DelayMs

Write-Host "`n[4/4] CROSS exact single-To-do matrix..." -ForegroundColor Yellow
$CrossAtoB = Invoke-ExactProbe -ClientId $ActorA.client_id -Url $TodoB -Marker $TodoMarkerB -Label "A / Chrome -> B exact TODO"
$CrossBtoA = Invoke-ExactProbe -ClientId $ActorB.client_id -Url $TodoA -Marker $TodoMarkerA -Label "B / Brave -> A exact TODO"

$ForeignAtoB = Test-Marker -Result $CrossAtoB -Marker $TodoMarkerB
$ForeignBtoA = Test-Marker -Result $CrossBtoA -Marker $TodoMarkerA
...
Write-Host ""
Write-Host "================ EXACT TODO AUTH MATRIX ================" -ForegroundColor Cyan
@(
    [pscustomobject]@{ Actor = "A / Chrome"; Owner = "A"; MarkerVisible = $TodoPassA; Verdict = "BASELINE_VISIBLE"; FinalUrl = $TodoResultA.final_url },
    [pscustomobject]@{ Actor = "B / Brave"; Owner = "B"; MarkerVisible = $TodoPassB; Verdict = "BASELINE_VISIBLE"; FinalUrl = $TodoResultB.final_url },
...
$ResultPath = Join-Path $OutDir "todo-strict-exact-auto-$Stamp.json"

$Report = [ordered]@{
    created_at = (Get-Date).ToUniversalTime().ToString("o")
    mode = "real-extension-profile-strict-exact-todo-readonly"
    canonical = @{
        A = @{ url = $TodoA; marker = $TodoMarkerA }
        B = @{ url = $TodoB; marker = $TodoMarkerB }
    }
    ...
    todo_baseline = @{ A = $TodoResultA; B = $TodoResultB }
...
if ($Finding) {
    ...
    Write-Host "Foreign marker gerçek browser profilinde exact To-do detail üzerinde görüldü." -ForegroundColor Red
} else {
    Write-Host "STRICT EXACT TODO MATRIX TEMİZ" -ForegroundColor Green
    Write-Host "İki canonical /todos/<id> resource cross kontrolde de foreign marker göstermedi."
}
