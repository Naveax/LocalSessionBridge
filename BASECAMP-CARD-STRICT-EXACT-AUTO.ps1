...
# Canonical single-card URLs confirmed by Basecamp's own activity mails.
$CardA = "https://app.basecamp.com/6259481/buckets/48506183/card_tables/cards/10203342033"
$CardB = "https://app.basecamp.com/6259488/buckets/48506260/card_tables/cards/10203434764"
$CardMarkerA = "H1-A-CARD-001"
$CardMarkerB = "H1-B-CARD-001"
...
Write-Host "BASECAMP CARD STRICT EXACT RESOURCE AUTH MATRIX" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "A = Chrome / 6259481 / project 48506183"
Write-Host "B = Brave  / 6259488 / project 48506260"
Write-Host "Discovery yok. İki canonical /card_tables/cards/<id> URL doğrudan test edilir." -ForegroundColor DarkGray
...
if (-not ($ProjectPassA -and $ProjectPassB)) {
    throw "PROJECT BASELINE başarısız. Exact Card/cross çalıştırılmadı."
}

Start-Sleep -Milliseconds $DelayMs

Write-Host "`n[3/4] Exact single-Card baseline..." -ForegroundColor Cyan
Write-Host "  A URL: $CardA"
Write-Host "  B URL: $CardB"

$CardResultA = Invoke-ExactProbe -ClientId $ActorA.client_id -Url $CardA -Marker $CardMarkerA -Label "A -> A exact CARD"
$CardResultB = Invoke-ExactProbe -ClientId $ActorB.client_id -Url $CardB -Marker $CardMarkerB -Label "B -> B exact CARD"

$CardPassA = Test-Marker -Result $CardResultA -Marker $CardMarkerA
$CardPassB = Test-Marker -Result $CardResultB -Marker $CardMarkerB

Write-Host "  A -> A CARD marker=$CardPassA final=$($CardResultA.final_url)"
Write-Host "  B -> B CARD marker=$CardPassB final=$($CardResultB.final_url)"

if (-not ($CardPassA -and $CardPassB)) {
    throw "EXACT CARD BASELINE başarısız. Cross-account jobs oluşturulmadı."
}

Start-Sleep -Milliseconds $DelayMs

Write-Host "`n[4/4] CROSS exact single-Card matrix..." -ForegroundColor Yellow
$CrossAtoB = Invoke-ExactProbe -ClientId $ActorA.client_id -Url $CardB -Marker $CardMarkerB -Label "A / Chrome -> B exact CARD"
$CrossBtoA = Invoke-ExactProbe -ClientId $ActorB.client_id -Url $CardA -Marker $CardMarkerA -Label "B / Brave -> A exact CARD"

$ForeignAtoB = Test-Marker -Result $CrossAtoB -Marker $CardMarkerB
$ForeignBtoA = Test-Marker -Result $CrossBtoA -Marker $CardMarkerA
...
Write-Host ""
Write-Host "================ EXACT CARD AUTH MATRIX ================" -ForegroundColor Cyan
@(
    [pscustomobject]@{ Actor = "A / Chrome"; Owner = "A"; MarkerVisible = $CardPassA; Verdict = "BASELINE_VISIBLE"; FinalUrl = $CardResultA.final_url },
    [pscustomobject]@{ Actor = "B / Brave"; Owner = "B"; MarkerVisible = $CardPassB; Verdict = "BASELINE_VISIBLE"; FinalUrl = $CardResultB.final_url },
...
$ResultPath = Join-Path $OutDir "card-strict-exact-auto-$Stamp.json"

$Report = [ordered]@{
    created_at = (Get-Date).ToUniversalTime().ToString("o")
    mode = "real-extension-profile-strict-exact-card-readonly"
    canonical = @{
        A = @{ url = $CardA; marker = $CardMarkerA }
        B = @{ url = $CardB; marker = $CardMarkerB }
    }
    ...
    card_baseline = @{ A = $CardResultA; B = $CardResultB }
...
if ($Finding) {
    ...
    Write-Host "Foreign marker gerçek browser profilinde exact Card detail üzerinde görüldü." -ForegroundColor Red
} else {
    Write-Host "STRICT EXACT CARD MATRIX TEMİZ" -ForegroundColor Green
    Write-Host "İki canonical /card_tables/cards/<id> resource cross kontrolde de foreign marker göstermedi."
}
